...
impl OpenFileMap {
    ...
    pub fn get(&self, fd: i32) -> Option<Arc<Mutex<OpenFile>>>{
        if let Some(f) = self.files_.lock().unwrap().get(&fd){
            ...
            error_msg("client::open_file_map::get".to_string(), "file descriptor not found".to_string());
            None
        }
    }
    pub fn get_dir(&self, dirfd: i32) -> Option<Arc<Mutex<OpenFile>>>{
        if let Some(f) = self.get(dirfd){
            ...
            error_msg("client::open_file_map::get_dir".to_string(), "file descriptor not found".to_string());
            None
        }
    }
    ...
    pub fn safe_generate_fd_idx(&mut self) -> i32{
        ...
        if self.fd_validation_needed_.load(std::sync::atomic::Ordering::Relaxed){
            while self.exist(fd){
    ...
    pub fn remove(&mut self, fd: i32) -> bool{
        ...
        if !self.exist(fd){
            ...
            (*map).remove(&fd);
            if self.fd_validation_needed_.load(std::sync::atomic::Ordering::Relaxed) && (*map).capacity() == 0{
    ...
    pub fn dup(&mut self, oldfd: i32) -> i32{
        if let Some(open_file)= self.get(oldfd){
            ...
            error_msg("clent::simplefs_openfile::openfilemap::dup".to_string(), "no such file".to_string());
            return -1;
        }
    }
