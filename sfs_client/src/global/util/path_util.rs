...
pub fn absolute_to_relative(mut root_path: String, absolute_path: String) -> String{
    if !is_absolute(&root_path) || !is_absolute(&absolute_path){
        error_msg("global::util::path_util::absolute_to_relative".to_string(), "argumants must be absolute path".to_string());
        return "".to_string()
    }
    if !absolute_path.starts_with(&root_path.clone()){
        error_msg("global::util::path_util::absolute_to_relative".to_string(), "the second path must start with the first path".to_string());
        return "".to_string()
    }
    ...
    if !is_absolute(&sliced){
        error_msg("global::util::path_util::absolute_to_relative".to_string(), "the first argument is not a complete path".to_string());
        return "".to_string()
    }
...
pub fn dirname(path: &String) -> String{
    ...
    if path.len() == 1 && !is_absolute(&path){
        error_msg("global::util::path_util::dirname".to_string(), "path can't be a single character".to_string());
        return "".to_string();
    }
