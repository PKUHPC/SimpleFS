...
pub(crate) async fn recver_client(
    ...
    unsafe {
        let mut ctx: *mut ReceiverClientContext = libc::calloc(1, std::mem::size_of::<ReceiverClientContext>()).cast();
        let init_ctx = ReceiverClientContext::new();
        libc::memcpy(ctx.cast(), (&init_ctx) as *const ReceiverClientContext as *const c_void, std::mem::size_of::<ReceiverClientContext>());
        (*ctx).addr = task.addr;
...
fn on_completion(wc: *mut ibv_wc, pd: *mut ibv_pd, _op: &ChunkOp) -> Result<i64, i32> {
    unsafe {
        ...
        let cm_ctx = (*id).context as *mut RDMACMContext;
...
fn post_receive_data(id: *mut rdma_cm_id) {
    unsafe {
        let mut wr: ibv_recv_wr = std::mem::zeroed();
        let mut bad_wr: *mut ibv_recv_wr = null_mut();
        wr.wr_id = id as u64;
