...
use super::{context::{ClientContext, RelativizeStatus}, syscall::{sfs_open, sfs_stat, stat, sfs_read, sfs_pread, sfs_write, sfs_pwrite, sfs_rmdir, sfs_remove, sfs_access, sfs_lseek, sfs_truncate, sfs_dup, sfs_dup2, sfs_getdents, sfs_getdents64, sfs_create}, util::get_metadata, path::{set_cwd, unset_env_cwd, get_sys_cwd}};
...
pub extern "C" fn hook_openat(dirfd: i32, path: * const c_char, mode: u32, flag: i32) -> i32{
    ...
    let res = ClientContext::get_instance().relativize_fd_path(dirfd, raw_path, false);
    let rstatus = res.0;
...
pub extern "C" fn hook_close(fd: i32) -> i32{
    if ClientContext::get_instance().get_ofm().lock().unwrap().exist(fd){
        ClientContext::get_instance().get_ofm().lock().unwrap().remove(fd);
        return 0;
    }
    if ClientContext::get_instance().is_internel_fd(fd){
        return 0;
    }
...
pub extern "C" fn hook_stat(path: * const c_char, buf: * mut stat) -> i32{
    let raw_path = unsafe { CStr::from_ptr(path).to_string_lossy().into_owned() };
    let res = ClientContext::get_instance().relativize_path(&raw_path, false);
...
pub extern "C" fn hook_fstat(fd: i32, buf: * mut stat) -> i32{
    if ClientContext::get_instance().get_ofm().lock().unwrap().exist(fd){
        let path = ClientContext::get_instance().get_ofm().lock().unwrap().get(fd).unwrap().lock().unwrap().get_path().clone() + "\0";
...
pub extern "C" fn hook_ftruncate(fd: i32, length: i64) -> i32{
    if ClientContext::get_instance().get_ofm().lock().unwrap().exist(fd){
        let path = ClientContext::get_instance().get_ofm().lock().unwrap().get(fd).unwrap().lock().unwrap().get_path().clone();
...
pub extern "C" fn hook_dup2(oldfd: i32, newfd: i32) -> i32{
    if ClientContext::get_instance().get_ofm().lock().unwrap().exist(oldfd){
...
pub extern "C" fn hook_symlinkat(oldname: *const c_char, newdfd: i32, newname: *const c_char) -> i32{
    ...
    let old_res = ClientContext::get_instance().relativize_path(&old_raw_path, false);
    ...
    let new_res = ClientContext::get_instance().relativize_fd_path(newdfd, new_raw_path, false);
...
pub extern "C" fn hook_chdir(path: *const c_char) -> i32{
    ...
    if res.0{
        ...
        rel_path = ClientContext::get_instance().get_mountdir().clone() + &rel_path;
...
pub extern "C" fn hook_fchdir(fd: i32) -> i32{
    if ClientContext::get_instance().get_ofm().lock().unwrap().exist(fd){
        ...
        let mut new_path = ClientContext::get_instance().get_mountdir().clone() + opendir.lock().unwrap().get_path();
        if has_trailing_slash(&new_path){
            ...
            unset_env_cwd();
            ClientContext::get_instance().set_cwd(get_sys_cwd());
        }
    }
    return 0;
}
