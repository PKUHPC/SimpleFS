...
impl ChunkOp {
    pub fn submit(&self, chunk: ChunkInfo) -> Result<i64, i32> {
        let buffer_offset = if chunk.chunk_id == self.chunk_start {
            0
        } else {
            CHUNK_SIZE * (chunk.chunk_id - self.chunk_start) - self.offset
        };
        let size = if chunk.chunk_id == self.chunk_start {
            u64::min(CHUNK_SIZE - self.offset, self.size)
        } else {
            u64::min(CHUNK_SIZE, self.size - buffer_offset)
        };
        //println!("{} - {}: {} {} | {} {}", self.chunk_start, chunk.chunk_id, self.offset, self.size, buffer_offset, size);
        let offset = if chunk.chunk_id == self.chunk_start {
            self.offset
        } else {
            0
        };
